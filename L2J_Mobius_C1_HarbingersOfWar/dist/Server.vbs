...
If path = "" Then
	MsgBox "Could not find JAVA_HOME environment variable!", vbOKOnly, "Server"
Else
...
'Generate command.
command = path & "java " & parameters & " -jar Server.jar"
If window = 1 Then
	command = "cmd /c start ""L2J Mobius - Server Console"" " & command
End If
...
Do
	...
	If exitcode <> 0 And exitcode <> 2 Then	'0 Terminated - 2 Restarted
		MsgBox "Server terminated abnormally!", vbOKOnly, "Server"
	End If
