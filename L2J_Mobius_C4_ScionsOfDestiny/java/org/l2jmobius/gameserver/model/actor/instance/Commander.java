 ...
 * @author programmos
 */
public class Commander extends Attackable
{
	private int _homeX;
	private int _homeY;
	private int _homeZ;
	
	public Commander(int objectId, NpcTemplate template)
	{
	...
	public boolean isAutoAttackable(Creature attacker)
	{
		...
		return (attacker instanceof Player) && (getFort() != null) && (getFort().getFortId() > 0) && getFort().getSiege().isInProgress() && !getFort().getSiege().checkIsDefender(((Player) attacker).getClan());
	}
	...
	public void addDamageHate(Creature attacker, int damage, int aggro)
	{
		...
		if (!(attacker instanceof Commander))
		{
