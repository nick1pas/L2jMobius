...
{
	protected static final Logger LOGGER = Logger.getLogger(Servitor.class.getName());
	...
	public Servitor(int objectId, NpcTemplate template, Player owner, Skill skill)
	{
	...
	public void reduceCurrentHp(int damage, Creature attacker)
	{
		...
		{
			sm.addString(((Npc) attacker).getTemplate().getName());
		}
		else
		{
	...
	private static class SummonLifetime implements Runnable
	{
		...
		SummonLifetime(Player player, Servitor newpet)
		{
			_player = player;
			_summon = newpet;
		}
