...
public abstract class AbstractItemPacket extends AbstractMaskPacket<ItemListType>
{
	...
	protected void writeItem(ItemInfo item)
	{
		...
		{
			writeInt(item.getPetData().getEvolve().ordinal()); // petEvolve step
			writeInt(0); // petName id
			writeInt(2);
			writeInt(3);
			writeInt(PetDataTable.getInstance().getTypeByIndex(item.getPetData().getIndex())); // pet id
			writeLong(item.getPetData().getExp()); // pet exp
		}
